/* ===== Auth & Login ===== */
    ...
    function initAuthWatch() {
      ...
      window._fb.onAuth(function(user) {
        ...
        if (role === 'doctor') { location.replace('../doctor/app.html'); return; }   // حساب طبيب على لوحة الممرضة → وجّهه للوحته
        if (role !== 'nurse') { location.replace('index.html'); return; }   // غير مصرّح → صفحة الدخول
...
/* ===== Main App ===== */
    ...
    setInterval(function () {
      ...
      try {
        ...
        if (typeof renderBothAppointmentColumns === 'function') renderBothAppointmentColumns();
    ...
    function closeMobFab() {
      ...
      _nurseFabScrim(false);
    }
    // طبقة تغبيش خلف قائمة الزرّ العائم — تبرز عناصر القائمة ونقرة عليها تُغلق
    function _nurseFabScrim(show) {
      var s = document.getElementById('nurseFabScrim');
      if (show) {
        if (!s) {
          s = document.createElement('div');
          s.id = 'nurseFabScrim';
          s.addEventListener('click', function() { closeFabDial(); closeMobFab(); });
          document.body.appendChild(s);
        }
        requestAnimationFrame(function() { s.classList.add('show'); });
      } else if (s) {
        s.classList.remove('show');
        setTimeout(function() { var el = document.getElementById('nurseFabScrim'); if (el && !el.classList.contains('show')) el.remove(); }, 220);
      }
    }
    ...
    // ── الشريط الجانبي: صار كبسولة أيقونات عائمة (بلا طيّ) ──
    var RAIL_KEY = 'nurseRailExpanded';
    window.toggleRail = function() {};   // لا شيء — الكبسولة ثابتة العرض

    // قائمة الملف الشخصي المنسدلة أسفل الكبسولة
    ...
    function appointmentCardHTML(record, status) {
      ...
      // بنية كرت ملف المريض نفسها: عين بالزاوية + اسم + معلومة + تاريخ + أزرار دائرية
      const waCall = `
        <a class="pb-fbtn pb-fbtn-wa" href="https://wa.me/${phone}" target="_blank" title="واتساب" onclick="event.stopPropagation()">${ICON.whatsapp}</a>
        <a class="pb-fbtn" href="tel:${phone}" title="اتصال" onclick="event.stopPropagation()">${ICON.phone}</a>`;
      const acts = isPending
        ? `<button class="pb-fbtn af-ok" title="قبول" onclick="event.stopPropagation();acceptAppointment('${record.id}')">${ICON.check}</button>
           <button class="pb-fbtn pb-fbtn-del" title="رفض" onclick="event.stopPropagation();rejectAppointment('${record.id}')">${ICON.x}</button>
           ${waCall}`
        : `${waCall}
           <button class="pb-fbtn pb-fbtn-del" title="إلغاء" onclick="event.stopPropagation();cancelAppointment('${record.id}')">${ICON.trash}</button>`;

      return `<div class="appt-card pb-folder ${statusCls}" data-card-id="${record.id}" title="اضغط للتفاصيل" onclick="openModalById('${record.id}')">
        <button class="pb-fmenu" title="تفاصيل" onclick="event.stopPropagation();openModalById('${record.id}')">${ICON.eye}</button>
        <div class="pb-fname">${escapeHtml(name)}</div>
        <div class="pb-fcount">${slot} · ${escapeHtml(visitType)}</div>
        <div class="pb-fmeta">${date}</div>
        <div class="pb-fact">${acts}</div>
      </div>`;
    }
    ...
    function renderPatientBook() {
      ...
      grid.innerHTML = '<div style="text-align:center;padding:32px;color:var(--text-muted)"><i class="fas fa-circle-notch fa-spin"></i> جارٍ التحميل...</div>';
      ...
      _pbTimer = setTimeout(function() {
        window._fb.getDocs(window._fb.col('patients'))
          .then(function(snap){
            ...
            var cur = (patientSearchQuery || '').trim();
    ...
    // آخر زيارة — نصّ نسبي (مطابق للطبيب)
    function _pbLastVisit(p) {
      ...
      try {
        ...
        return { txt: 'آخر زيارة ' + t };
      } catch (e) { return { txt: formatDateAr(d) }; }
    }
    function _pbRenderRows(patients) {
      const grid = document.getElementById('patientsGrid');
      if (!patients.length) {
        grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:32px;color:var(--text-muted);"><i class="fas fa-user-slash" style="font-size:2rem;display:block;margin-bottom:10px;opacity:.4;"></i>لا يوجد مرضى</div>';
        return;
      }
      // كروت مطابقة لدفتر مرضى الطبيب: عين للعرض بالزاوية + الاسم + عدد الزيارات + آخر زيارة + إجراءات
      grid.innerHTML = patients.map(p=>{
        const phone = normalizePhone(p.phone);
        const lv = _pbLastVisit(p);
        return `<div class="pb-folder" title="اضغط لعرض الإضبارة" onclick="openPatientDetailsModal('${p.id}')" oncontextmenu="event.preventDefault();openPatientDetailsModal('${p.id}');return false;">
          <button class="pb-fmenu" title="عرض الإضبارة" onclick="event.stopPropagation();openPatientDetailsModal('${p.id}')"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M2.5 12S6 5.5 12 5.5 21.5 12 21.5 12 18 18.5 12 18.5 2.5 12 2.5 12Z"/><circle cx="12" cy="12" r="3"/></svg></button>
          ...
          <div class="pb-fact">
            <button class="pb-fbtn" title="إضافة زيارة" onclick="event.stopPropagation();openAddVisitModal('${p.id}','${escapeHtml(p.name)}','${escapeHtml(p.phone)}','${p.birthDate||''}','${escapeHtml(p.address||'')}')"><i class="fas fa-plus"></i></button>
            <a class="pb-fbtn pb-fbtn-wa" href="https://wa.me/${phone}" target="_blank" title="واتساب" onclick="event.stopPropagation()"><i class="fab fa-whatsapp"></i></a>
            <a class="pb-fbtn" href="tel:${phone}" title="اتصال" onclick="event.stopPropagation()"><i class="fas fa-phone"></i></a>
            <button class="pb-fbtn pb-fbtn-del" title="حذف" onclick="event.stopPropagation();deletePatient('${p.id}')"><i class="fas fa-trash-alt"></i></button>
          </div>
        </div>`;
      }).join('');
    }
    ...
    function renderChartVisits(pid) {
      ...
      // الممرضة: تاريخ ونوع الزيارة فقط — بدون أي تشخيص أو وصفة. شكل الفولدر مطابق لتطبيق الطبيب.
      box.innerHTML = visits.map(function(o){
        const v=o.v, c=schedStatusColor(v);
        return '<div class="visit-folder" style="--vf-accent:' + c.bd + ';">'
          + '<span class="vf-inner">'
          +   '<span class="vf-back"></span>'
          +   '<span class="vf-paper"></span>'
          +   '<span class="vf-front">'
          +     '<span class="vf-title">' + escapeHtml(v.visitType || 'زيارة') + '</span>'
          +     '<span class="vf-meta">' + formatDateAr(v.date) + '</span>'
          +     '<span class="vf-type">' + (slotTimeOf(v) || '—') + '</span>'
          +   '</span>'
          + '</span>'
          + '</div>';
      }).join('');
    ...
    /* ── نموّ الحقول مع الكتابة ──
       ...
       (كان min-height:56px من زمن الصناديق فيبقى فراغ بين التسمية والخطّ.) */
    ...
    window.openPatientInfoEditor = function(pid) {
      ...
      // scrollHeight للعنصر المخفيّ صفر، فلا يُقاس إلا بعد الرسم
      requestAnimationFrame(function() {
    ...
    function _vmRenderVisitSearch(matches) {
    ...
    document.getElementById('patientSearchInput').addEventListener('input', (e)=>{
      ...
      const localMatch = () => Object.values(allPatients).filter(p=> (p.name||'').toLowerCase().includes(term) || (p.phone||'').includes(raw));
      _vmRenderVisitSearch(localMatch());   // نتائج محلية فورية
      // بحث من الخادم (اسم بالبادئة أو هاتف) ليشمل العائدين خارج الأربعين المحمَّلين
      clearTimeout(_vmSearchTimer);
      _vmSearchTimer = setTimeout(function(){
        const isPhone = /^[\d+]{3,}$/.test(raw);
        const q = isPhone
          ? window._fb.query(window._fb.col('patients'), window._fb.where('phone','==', raw))
          : window._fb.query(window._fb.col('patients'), window._fb.orderBy('name'), window._fb.where('name','>=', raw), window._fb.where('name','<=', raw + ''), window._fb.limit(25));
        window._fb.getDocs(q).then(function(snap){
          snap.forEach(function(d){ if(!allPatients[d.id]) allPatients[d.id] = Object.assign({id:d.id}, d.data()); });
          if (document.getElementById('patientSearchInput').value.trim() === raw) {
            const seen={}, merged=[];
            localMatch().forEach(function(p){ if(!seen[p.id]){ seen[p.id]=1; merged.push(p); } });
            _vmRenderVisitSearch(merged);
          }
        }).catch(function(err){ console.error('[vmSearch]', err); });
      }, 300);
    });
    ...
    function submitNewPatientVisit() {
      ...
      // منع التكرار: تحقّق محلّي (بصيغة موحّدة، متسامح مع اختلاف الصيغة) + من الخادم بالهاتف
      const nph = normalizePhone(phone);
      const localDup = Object.values(allPatients).find(function(p) { return nph && normalizePhone(p.phone || '') === nph; }) || null;
      window._fb.getDocs(window._fb.query(window._fb.col('patients'), window._fb.where('phone', '==', phone)))
        .then(function(snap) {
          var dup = localDup;
          snap.forEach(function(d) { var o = d.data() || {}; if (!dup && normalizePhone(o.phone || '') === nph) dup = Object.assign({ id: d.id }, o); });
          _dupDecision(dup, name, phone, birth, addr, type, slot, visitDate);
        })
        .catch(function(e) { console.error('[dupCheck]', e); _dupDecision(localDup, name, phone, birth, addr, type, slot, visitDate); });
    }
    // نبّه إن وُجد مريض بنفس الرقم (لا إنشاء تلقائي)، وإلا أنشئ الجديد
    function _dupDecision(dup, name, phone, birth, addr, type, slot, visitDate) {
      if (!dup) { _createNewPatientVisit(name, phone, birth, addr, type, slot, visitDate); return; }
      // أخفِ فورم إدارة الزيارة ليظهر التنبيه بالمنتصف نظيفاً
      document.getElementById('visitManagementModal').classList.add('hidden');
      appConfirm('⚠️ يوجد مريض مسجّل بنفس رقم الهاتف: «' + (dup.name || '—') + '». هل أنت متأكّد من إنشاء اضبارة جديدة منفصلة؟', 'إنشاء جديد رغم ذلك')
        .then(function(ok) {
          if (ok) { _createNewPatientVisit(name, phone, birth, addr, type, slot, visitDate); }
          else {
            // ألغِ → أعِد فتح الفورم على اضبارة المريض الموجود
            allPatients[dup.id] = dup;
            document.getElementById('visitManagementModal').classList.remove('hidden');
            showVisitStep(3); previewPatientForVisit(dup.id);
          }
        });
    }
    // الإنشاء الفعلي + تسجيل الزيارة (بعد اجتياز فحص التكرار)
    function _createNewPatientVisit(name, phone, birth, addr, type, slot, visitDate) {
...
/* ===== Message Customizer ===== */
    ...
    /* شريط رأس الاضبارة — تنبيهات الخطر فقط (مطابق لتطبيق الطبيب).
       كان يعرض العمر/الزمرة/الزيارات/الميلاد/العنوان، وكلها مكرّرة حرفياً في
       بطاقة «معلومات المريض» أسفله: خمس شارات بصفر معلومة جديدة. صار مكانها
       الحساسية والأمراض المزمنة — وإن لم يوجد خطر يختفي الشريط. */
    ...
    function buildCustomFieldInputs(container, fields, values, opts) {
      ...
      fields.forEach(function(f) {
        ...
        if (f.type === 'textarea') {
          ...
          el = document.createElement('textarea'); el.rows = 1; el.className = 'form-input'; el.value = (val != null ? val : '');
